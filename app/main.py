...
def verify_credentials(credentials: HTTPBasicCredentials = Depends(security)):
    """Verify username/password against Render environment variables"""
...
async def search_places(req: SearchRequest, username: str = Depends(verify_credentials)) -> SearchResponse:
...
async def search_places_next(token: str = Query(..., description="Upstream Places API nextPageToken"), username: str = Depends(verify_credentials)) -> SearchResponse:
    reload_categories()
    if not PLACES_API_KEY:
        raise HTTPException(status_code=500, detail="PLACES_API_KEY not configured")
...
# Categories endpoint - protected
@app.get("/categories")
def get_categories(username: str = Depends(verify_credentials)) -> List[Dict[str, Any]]:
    reload_categories()
...
async def search_places_csv(
    ...
    resp = await search_places(req, username)
    output = io.StringIO()
...
# Place details enrichment - protected
@app.post("/places/details")
async def places_details(payload: Dict[str, Any], username: str = Depends(verify_credentials)) -> Dict[str, Any]:
    if not PLACES_API_KEY:
        raise HTTPException(status_code=500, detail="PLACES_API_KEY not configured")
...
# CRM import endpoint - protected (placeholder for your CRM implementation)
@app.post("/prospects/import-from-search")
async def import_prospects(payload: Dict[str, Any], username: str = Depends(verify_credentials)) -> Dict[str, Any]:
    """Import prospects from search results to CRM"""
    prospects = payload.get("prospects", [])
    if not isinstance(prospects, list) or not prospects:
        raise HTTPException(status_code=400, detail="prospects array required")
    
    # TODO: Implement your CRM import logic here
    # For now, just return success
    imported_count = len(prospects)
    duplicates_count = 0
...
@app.get("/crm")
def serve_crm(username: str = Depends(verify_credentials)):
    crm_path = os.path.join(WEB_DIR, "crm.html")
    if os.path.exists(crm_path):
        return FileResponse(crm_path)
    return {"message": "CRM interface not found"}
